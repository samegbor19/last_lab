...
def ik_service_client():
    ...
    usr_input = []
    for usr_data in range(0,3): 
        x = input("Please provide a coordinate:")
        x = float(x)
        usr_input.append(x) 
    pose_stamped.pose.position.x = float(usr_input[0])
    pose_stamped.pose.position.y = float(usr_input[1])
    pose_stamped.pose.position.z = float(usr_input[2])    
