...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    for rows in range(new_gray.shape[0]):
        for columns in range(new_gray.shape[1]): 
            if new_gray[rows][columns] < upper_thresh and new_gray[rows][columns] > lower_thresh: 
                new_gray[rows][columns] = 0 
            else: 
                new_gray[rows][columns] = 1 
    return new_gray      
