...
class OccupancyGrid2d(object):
    ...
    def LoadParameters(self):
        ...
        # -- self._x_num
        # if not rospy.has_param("~x_num"):
        #     return False
        self._x_num = rospy.get_param("~x/num")
        # -- self._x_min
        # if not rospy.has_param("~x_min"):
        #     return False
        self._x_min = rospy.get_param("~x/min")
        # -- self._x_max
        # if not rospy.has_param("~x_max"):
        #     return False
        ...
        self._x_res = float((self._x_max - self._x_min)/ (self._x_num))
        # -- self._y_num
        # if not rospy.has_param("~y_num"):
        #     return False
        self._y_num = rospy.get_param("~y/num")
        # -- self._y_min
        # if not rospy.has_param("~y_min"):
        #     return False
        self._y_min = rospy.get_param("~y/min")
        # -- self._y_max
        # if not rospy.has_param("~y_max"):
        #     return False
        ...
        self._y_res = float((self._y_max - self._y_min)/ (self._y_num))
        ...
        # self._fixed_frame = rospy.get_param("odom")
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            t_agl = idx*msg.angle_increment+ msg.angle_min + yaw
            ...
            #assuming angle in rad 
            # import pdb
            # pdb.set_trace()
            xi =  r * np.cos(t_agl) + sensor_x
            yi = r * np.sin(t_agl) + sensor_y
            grid_x, grid_y = self.PointToVoxel(xi,yi)
            self._map[grid_x, grid_y] = min(self._occupied_threshold, self._map[grid_x, grid_y] + self._occupied_update)
            lastvoxel = (grid_x, grid_y)
            for incr in np.arange(0,r,(self._x_res**2 + self._y_res**2)**(1/2)): 
                
                xi =  incr * np.cos(t_agl) + sensor_x
                yi = incr * np.sin(t_agl) + sensor_y
                grid_x, grid_y = self.PointToVoxel(xi,yi)
                # if incr == r:
                #     self._map[grid_x, grid_y] = min(self._occupied_threshold, self._map[grid_x, grid_y] + self._occupied_update)
                # else:
                if (grid_x, grid_y) != lastvoxel:
                    lastvoxel = (grid_x, grid_y)
